...
const VERSION = '5.0.0';
const logger = debugModule('node-bigcommerce-esm:bigcommerce');
  ...
  run(method, path, data) {
    ...
    const options = {
      ...
      headers: Object.assign(
        {
          'User-Agent': 'node-bigcommerce-esm/' + VERSION,
            ...
            if (shouldUnzip) {
              ...
              return unzip(Buffer.from(body, encoding), (err, data) => {
                ...
                const error = new Error(
                  `Request returned error code: ${res.statusCode} and body: ${data.toString('utf8')}`
                );
